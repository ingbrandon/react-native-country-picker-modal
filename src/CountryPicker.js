...
export default class CountryPicker extends Component {
  ...
  static renderEmojiFlag(cca2, emojiStyle, imageFlagStyle) {
    {/* Se modifico  y se agrego imageFlagStyle el 13/10/2019 */ }
    return (
      <Text style={[countryPickerStyles.emojiFlag, emojiStyle, imageFlagStyle]} allowFontScaling={false}>
  ...
  static renderImageFlag(cca2, imageStyle, imageFlagStyle) {
    {/* Se modifico  y se agrego imageFlagStyle el 13/10/2019 */ }
    return cca2 !== '' ? (
      <Image
        style={[countryPickerStyles.imgStyle, imageStyle, imageFlagStyle]}
        source={{ uri: countries[cca2].flag }}
      />
    ) : null
  }

  static renderFlag(cca2, itemStyle, emojiStyle, imageStyle, imageFlagStyle) {
    {/* Se modifico  y se agrego imageFlagStyle el 13/10/2019 */ }
    return (
      <View style={[countryPickerStyles.itemCountryFlag, itemStyle]}>
        {isEmojiable
          ? CountryPicker.renderEmojiFlag(cca2, emojiStyle, imageFlagStyle)
          : CountryPicker.renderImageFlag(cca2, imageStyle, imageFlagStyle)}
      </View>
    )
  }
  ...
  renderLetters(letter, index) {
    {/* Se modifico  y se agrego letterText el 13/10/2019 */ }
    return (
      <TouchableOpacity
        key={index}
      ...
      >
        <View style={styles.letter}>
          <Text style={[styles.letterText, this.props.letterText]} allowFontScaling={false}>
            {letter}
          </Text>
        </View>
      </TouchableOpacity>
  ...
  renderCountryDetail(cca2) {
    const { imageFlagStyle } = this.props; {/* Se modifico  y se agrego imageFlagStyle y countryname el 13/10/2019 */ }
    const country = countries[cca2]
    return (
      <View style={styles.itemCountry}>
        {!this.props.hideCountryFlag && CountryPicker.renderFlag(cca2, "", "", "", imageFlagStyle)}
        <View style={styles.itemCountryName}>
          <Text style={[styles.countryName, this.props.countryName]} allowFontScaling >
            {this.getCountryName(country)}
          </Text>
          {this.props.showCallingCode &&
            country.callingCode &&
            <Text style={[styles.countryName, this.props.countryName]}>{`+${country.callingCode}`}</Text>}
        </View>
      </View>
    )
  }
    ...
    return renderFilter ? (
      renderFilter({ value, onChange, onClose })
        ...
        <TextInput
          ...
          style={[styles.input, !this.props.closeable && styles.inputOnly, this.props.inputStyle]}
      ...
      <View style={styles.container}>
        ...
        >
          <SafeAreaView style={styles.modalContainer}>
            <View style={[styles.header, this.props.container]}>
              {this.props.closeable && (
                <CloseButton
                  image={this.props.closeButtonImage}
                  styles={[styles.closeButton, styles.closeButtonImage, this.props.closeButtonStyle]}
